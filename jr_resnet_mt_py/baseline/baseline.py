...
def runModelPar(runner: "Runner", inputs):
    ...
    inputData[0].flags.writeable = True
    outputData[0].flags.writeable = True

    for i in range(batch_size):
        inputData[0][:,:] = inputs[i][:,:]
        job_id = runner.execute_async(inputData, outputData)
        runner.wait(job_id)
        outputs[i][:,:] = outputData[0][:,:]
...
def main(task_num,batch_size):
    ...
    for i in range(task_num // batch_size):
        out = runModelPar(all_dpu_runners[0], data)
        outputs.append(out)
