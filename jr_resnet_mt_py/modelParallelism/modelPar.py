...
def runModelPar(runner: "Runner", runner1: "Runner", runner2: "Runner", inputs):
    ...
    """run with batch """
    inputData[0][0][:,:] = inputs[0][:,:]
    job_id = runner.execute_async(inputData[0], outputData[0])
    runner.wait(job_id)
    inputData[1][0][:,:] = outputData[0][0][:,:]
    inputData[0][0][:,:] = inputs[1][:,:]
    job_id = runner.execute_async(inputData[0], outputData[0])
    ...
    for i in range(2,batch_size):
        inputData[2][0][:,:] = outputData[1][0][:,:]
        inputData[1][0][:,:] = outputData[0][0][:,:]
        inputData[0][0][:,:] = inputs[i][:,:]
        job_id = runner.execute_async(inputData[0], outputData[0])
        job_id1 = runner1.execute_async(inputData[1], outputData[1])
    ...
    runner1.wait(job_id1)
    runner2.wait(job_id2)
    outputs[batch_size-2][:,:] = outputData[2][0][:,:]

    inputData[2][0][:,:] = outputData[1][0][:,:]
    job_id2 = runner2.execute_async(inputData[2], outputData[2])
    runner.wait(job_id2)
    outputs[batch_size-1][:,:] = outputData[2][0][:,:]
...
def main(task_num,batch_size):
    ...
    # sub-models
    model_num = 3
    xmodel_file = "./vck190_model_parallelism_int.xmodel"
